	...
	var templateHeader=function(object,option)
	{
		...
		this.build=function() 
		{
			...
			$(window).resize(function() 
			{
                ...
                if($menuResponsive.hasClass('template-state-open'))
                    $menuResponsive.animate({top:(parseInt(window.getComputedStyle(document.documentElement).getPropertyValue("--ql-bar-height"), 10) || 0)});
                else
                {
		...
		{
			$menuResponsive.animate({top:(parseInt(window.getComputedStyle(document.documentElement).getPropertyValue("--ql-bar-height"), 10) || 0)},{duration:500,easing:'easeInOutCubic',complete:function()
			{
